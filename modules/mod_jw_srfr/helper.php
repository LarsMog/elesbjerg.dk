...
class SimpleRssFeedReaderHelper {
		...
		foreach($feeds as $key=>$feed){
			...
			if(is_object($xml) && $items = $xml->xpath("/rss/channel/item")) {
				...
				*/foreach($items as $item){
					...
					$pos = strpos($item->description,'<div class="field field-name-body field-type-text-with-summary field-label-hidden');
					//echo 'POS: '.$pos;
					/*$item->description = str_replace('facebook</a>','',$item->description);
					$item->description = str_replace('twitter</a>','',$item->description);
					$item->description = str_replace('Udskriv</a>','',$item->description);*/
					$item->description = substr($item->description,$pos);
	...
	function wordLimiter($str,$limit=100,$end_char=' &#8230;'){
		...
		$str = strip_tags($str);
		$str = substr($str,strpos($str,' Udskriv')+8);
