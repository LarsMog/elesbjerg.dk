...
class modSlideshowckHelper {
        ...
        foreach ($items as $i => $item) {
            ...
            if (stristr($item->imgname, "http")) {
                ...
                if ($params->get('thumbnails', '1') == '1') {
                    $item->imgthumb = self::resizeImage($item->imgname, $params->get('thumbnailwidth', '100'), $params->get('thumbnailheight', '75'));
                } else {
    ...
    static function getItemsFromfolder(&$params) {
        ...
        foreach ($items as & $item) {
            ...
            $item->imgthumb = '';
            if ($params->get('thumbnails', '1') == '1') $item->imgthumb = self::resizeImage($item->imgname, $params->get('thumbnailwidth', '100'), $params->get('thumbnailheight', '75'));
            $thumbext = explode(".", $item->imgname);
            $thumbext = end($thumbext);
            // set the variables
            $item->imgvideo = null;
            ...
            if (!in_array(strToLower(JFile::getExt($item->imgname)),$authorisedExt)) continue;
    ...
    static function getImageDataFromfolder(&$item, $params) {
        ...
        if ($item->imgvideo) $item->slideselect = 'video';
    ...
    static function resizeImage($file, $x, $y) {
        ...
        if (!$file)
            return;
        $file = JPATH_ROOT . '/' . $file;//var_dump(JFile::getName($file));
        //$x = 100;
        ...
        if ($size) {
            ...
            $thumbfile = str_replace("." . $thumbext, "_th." . $thumbext, $thumbfile);//var_dump($thumbfile);
            $thumbfolder = str_replace(JFile::getName($file), "th/", $file);
            if (!JFolder::exists($thumbfolder)) JFolder::create($thumbfolder);

            if (JFile::exists($thumbfile)) {
                $thumbsize = getimagesize($thumbfile);
                if ($thumbsize[0] == $x AND $thumbsize[1] == $y) {
                    //echo 'miniature existante';
                    return str_replace(JPATH_ROOT, JUri::root(), $thumbfile);
                }
            }
